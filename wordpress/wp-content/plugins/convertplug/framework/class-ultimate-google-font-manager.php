...
if ( ! class_exists( 'Ultimate_Google_Font_Manager' ) ) {
	...
	class Ultimate_Google_Font_Manager {
		...
		public function admin_google_font_scripts() {
			wp_register_script( 'ultimate-google-fonts-script', SMILE_FRAMEWORK_URI . '/assets/js/google-fonts-admin.js', array( 'jquery' ), CP_VERSION, false );
			wp_enqueue_script( 'ultimate-google-fonts-script' );
			wp_register_style( 'ultimate-google-fonts-style', SMILE_FRAMEWORK_URI . '/assets/css/google-fonts-admin.css', array(), CP_VERSION );
		...
		public function enqueue_selected_ultimate_google_fonts() {
			...
			if ( ! empty( $selected_fonts ) ) {
				...
				wp_register_style( 'ultimate-selected-google-fonts-style', $font_api_call, array(), CP_VERSION );
		...
		public function enqueue_ultimate_google_fonts() {
			...
			if ( ! empty( $selected_fonts ) ) {
				...
				$link = 'https://fonts.googleapis.com/css?family=' . $font_call;
				wp_register_style( 'ultimate-selected-google-fonts-style', $link, array(), CP_VERSION );
				wp_enqueue_style( 'ultimate-selected-google-fonts-style' );
			}
		}
		...
		public function ultimate_font_manager_dashboard() {
			...
			<div class="wrap uavc-gfont">
				...
				<div class="nav">
					...
					<input type="text" id="search_gfont" name="search_gfont" placeholder="<?php echo esc_html__( 'Search font..', 'ultimate_vc' ); ?>"/>
				</div>
								...
								<div class="selected-font">
									...
									if ( ! empty( $sfont['variants'] ) || ! empty( $sfont['subsets'] ) ) :
										...
										<div class="selected-font-content">
											...
											?>
											<input type="button" class="button alignleft update-google-font-button" value="<?php echo esc_attr__( 'Update font', 'ultimate_vc' ); ?>" data-font_name="<?php echo esc_attr( $sfont['font_name'] ); ?>" />
				...
				if ( isset( $font_arr[1] ) ) {
					...
					if ( isset( $font_call_arr[1] ) && '' !== $font_call_arr[1] ) {
						...
						if ( ! wp_script_is( 'ultimate-' . $eq_name, 'registered' ) ) {
							wp_register_style( 'ultimate-' . $eq_name, $link, array(), CP_VERSION );
